...
> **最終更新:** Phase 4+ 完了 — FANZAトクナビ改名、ヒーロー改行修正、ホームページ短縮、絵文字→アイコン置換、テスト192件全パス
...
5. Twitter/Xアカウント作成（@fanza_tokunavi など）
...
2. D1データベース作成 — `npx wrangler d1 create fanza-tokunavi-db`
...
## 次フェーズ: 新機能ロードマップ（Phase 5）

### 🔴 第1週目に実装（収益直結）

| # | 機能 | 概要 | 技術 |
|---|------|------|------|
| 1 | お気に入り＆ウォッチリスト | 作品ブックマーク+値下げ通知 | localStorage + SW Push |
| 2 | 買い時判定バッジ | 過去セール傾向から「今買う/待つ」表示 | ビルド時にAPI算出 |
| 3 | 今日のおすすめ（デイリーピック） | 毎日1作品を自動選出+SNS共有 | ビルド時自動選出 |

### 🟡 第2週目に実装（差別化強化）

| # | 機能 | 概要 | 技術 |
|---|------|------|------|
| 4 | シリーズ完走ガイド | シリーズ全作品の時系列表示+コンプ率 | DMM APIシリーズデータ |
| 5 | コスパ計算機（分/円） | 収録時間÷価格でお得度ランキング | ビルド時計算 |
| 6 | セール予測カレンダー | 過去データからセール日を予測+グラフ | Workers D1 |

### 🟢 第3週目以降（長期差別化）

| # | 機能 | 概要 | 技術 |
|---|------|------|------|
| 7 | 価格履歴チャート（Keepa型） | 作品別の価格推移グラフ | D1 + Chart.js |
| 8 | SNS共有カード自動生成 | 作品情報を画像カードに自動変換 | Canvas API / Satori |
| 9 | パーソナライズドフィード | 閲覧履歴ベースの「あなたへのおすすめ」 | localStorage |
| 10 | ランキングバトル | 2作品対決の投票トーナメント | Workers + D1 |
| 11 | ガチャ風レコメンド | ランダムで作品を1本提案 | クライアントJS |
| 12 | みんなの節約術投稿 | UGC型のTips共有 | Workers + D1 |

### 実装の指示テンプレート
```
/Users/thisdevice/dmm-affiliate-site を開いて、Phase 5の新機能を実装して:

1. お気に入り＆ウォッチリスト機能（localStorageで作品保存、値下げ検知時にPush通知）
2. 買い時判定ツール（ビルド時にセール頻度を計算、作品カードに「今が買い時」バッジ表示）
3. 今日のおすすめ（ビルド時にレビュー評価+コスパで1作品自動選出、/daily-pickページ作成）

各機能はsrc/app/配下に新ページ作成、ホームページにリンク追加。
ビルドとテスト通してからGitLabにpush。
```
