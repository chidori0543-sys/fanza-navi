...
> **最終更新:** Phase 5 完了 — 12新機能追加、全emoji→react-icons、アフィリエイトリンク修正、ロゴ刷新、テスト192件全パス
...
11. [Phase 5 新機能（実装完了）](#phase-5-新機能--実装完了-)
12. [サイトの使い方ガイド](#サイトの使い方ガイド)
13. [これからの1週間の詳細運用フロー](#これからの1週間の詳細運用フロー)
14. [AIへの指示テンプレート](#aiへの指示テンプレート運用自動化用)
15. [Phase 6 ロードマップ](#次フェーズ-phase-6-ロードマップworkers本番化)
...
### ページ一覧（全41ページ + ジャンル/女優/メーカー動的ページ）
| URL | 内容 | 更新頻度 |
|-----|------|----------|
| `/` | トップページ（ランキング・新作・セール・全18ツール） | ビルド毎 |
...
| `/watchlist` | 🆕 お気に入り＆ウォッチリスト（値下げ追跡） | リアルタイム |
| `/daily-pick` | 🆕 今日のおすすめ（デイリーピック） | ビルド毎 |
| `/gacha` | 🆕 ガチャ風レコメンド（ランダム作品提案） | リアルタイム |
| `/cospa-calc` | 🆕 コスパ計算機（分/円ランキング） | ビルド毎 |
| `/buy-timing` | 🆕 買い時判定ツール | ビルド毎 |
| `/ranking-battle` | 🆕 ランキングバトル（2作品対決投票） | リアルタイム |
| `/series-guide` | 🆕 シリーズ完走ガイド（コンプ率表示） | ビルド毎 |
| `/sale-predict` | 🆕 セール予測カレンダー | 週1回 |
| `/savings-tips` | 🆕 みんなの節約術（UGC型Tips共有） | リアルタイム |
| `/price-history` | 🆕 価格履歴チャート（Keepa型） | ビルド毎 |
| `/sns-cards` | 🆕 SNS共有カード自動生成 | リアルタイム |
| `/personalized` | 🆕 パーソナライズドフィード | リアルタイム |
| `/genre/*` | ジャンル別一覧 | ビルド毎 |
| `/actress/*` | 女優別ページ | ビルド毎 |
| `/maker/*` | メーカー別ページ | ビルド毎 |
...
上の「これからの1週間の詳細運用フロー」セクションに統合済み。そちらを参照。
...
## 次フェーズ: Phase 6 ロードマップ（Workers本番化）
