...
| 変数名 | 説明 | 必須 |
|--------|------|------|
| `DMM_API_ID` | DMM API ID（DMM API利用時） | 任意 |
...
- Workers の `/api/search` `/api/gacha` 相当の full-catalog 母数は public FANZA GraphQL を使うため、`DMM_API_ID` が無効でも検索/抽選は復旧できます
...
```bash
# DMM API認証情報（price-history / sale bot など DMM API 利用時）
npx wrangler secret put DMM_API_ID

# アフィリエイト付き作品リンク生成に使う ID
...
- `/search` `/gacha` は Worker 側で public FANZA GraphQL を利用して復旧済み
