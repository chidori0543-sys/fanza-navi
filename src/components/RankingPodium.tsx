      ...
      <div className={isLeadCard ? "flex h-full flex-col" : ""}>
        ...
        >
          ...
          <div className="absolute right-3 top-3 z-10 flex items-center gap-2">
            <div className="rounded-full bg-black/45 px-2.5 py-1 text-xs font-semibold text-white/90">
        ...
        <div className="p-3 md:p-4">
          ...
          <div className={`${isLeadCard ? "mt-3" : "mt-2"} flex flex-wrap items-end justify-between gap-3`}>
            <div>
              <p className="text-[10px] tracking-[0.18em] text-[var(--color-text-muted)]">価格</p>
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {hasAffiliateUrl ? (
                ...
                <span className="inline-flex items-center justify-center rounded-full border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2 text-xs text-[var(--color-text-muted)]">
                  リンク準備中
                </span>
              )}
            </div>
  ...
  return (
    <div className="grid gap-2.5 md:grid-cols-[1.2fr_0.8fr]">
      {products.slice(0, 3).map((product, index) => (
        <PodiumCard key={product.id} product={product} index={index} />
      ))}
    </div>
  );
}
