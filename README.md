...
- 静的カタログ（21万件・FANZA GraphQL から build 時に生成）
...
```bash
# catalog があれば流用、なければ build 前に自動生成
npm run build

# catalog を強制再生成してから build
npm run build:full
...
catalog は `public/catalog/` に生成され、`npm run build` 実行時に不足していれば自動で補完されます。
